...
static int rodadasCPU(int N, int default_runs) { return default_runs; }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (a == "--mode" && i + 1 < argc)
      ...
      cout << "Uso: " << argv[0] << " [opcoes]\n\n"
           ...
           << "  --sizes <n1,n2,...>          Tamanhos N das matrizes (padrao: "
              "100,200,500,1000,2000,5000,10000)\n"
           << "  --runs <n>                   Repeticoes por teste (padrao: "
              "10, reduzido automaticamente para N grande)\n"
           << "  --no-validation              Desabilita validacao matematica "
              "GPU vs CPU\n"
           << "  --out <sufixo>               Sufixo personalizado para o "
              "arquivo CSV\n"
  ...
  for (int N : sizes) {
    ...
    // Aloca matrizes de entrada como pinned memory (otimiza transferências
    // PCIe)
    ...
    if (mode == "all" || mode == "gpu") {
      if (validation && mode == "gpu") {
        cout << "  [Validacao]      Gerando referencia CPU (OpenMP)... "
             << flush;
        memset(h_C, 0, bytes);
  ...
  cout
      ...
      << "====================================================================="
         "==================================================================\n"
      << "                                              TABELA RESUMO DO "
         "BENCHMARK (TEMPOS EM MS)\n"
      << "====================================================================="
         "==================================================================\n"
      << "   N   | CPU Seq    | CPU OMP    | CPU AVX2   | GPU Naive (Kernel / "
         "PCIe)     | GPU Tiled (Kernel / PCIe)     | Valido\n"
      << "-------+------------+------------+------------+----------------------"
         "---------+-------------------------------+--------\n";
  ...
  for (auto const &r : resultados) {
    ...
    cout << setw(6) << r.N << " | " << fmt_cpu(r.t_cpu_seq) << " | "
         << fmt_cpu(r.t_cpu_omp) << " | " << fmt_cpu(r.t_cpu_manual) << " | "
         << fixed << setprecision(2) << setw(8) << r.t_gpu_naive_total
         << " ms (" << setw(7) << r.t_gpu_naive_kernel << " / " << setw(5)
         << (r.t_gpu_naive_h2d + r.t_gpu_naive_d2h) << ") | " << setw(8)
         << r.t_gpu_tiled_total << " ms (" << setw(7) << r.t_gpu_tiled_kernel
  ...
  cout
      << "====================================================================="
         "==================================================================\n"
      << "Legenda: GPU = Total (Kernel puro / PCIe H2D+D2H)  |  N/A = teste "
         "pulado\n\n";
  ...
  csv << "N,CPU_Seq_ms,CPU_OMP_ms,CPU_Manual_ms,"
      << "GPU_Naive_H2D_ms,GPU_Naive_Kernel_ms,GPU_Naive_D2H_ms,GPU_Naive_"
         "Total_ms,"
      << "GPU_Tiled_H2D_ms,GPU_Tiled_Kernel_ms,GPU_Tiled_D2H_ms,GPU_Tiled_"
         "Total_ms,"
      << "Valido\n";
  for (auto const &r : resultados) {
    csv << r.N << "," << r.t_cpu_seq << "," << r.t_cpu_omp << ","
        << r.t_cpu_manual << "," << r.t_gpu_naive_h2d << ","
        << r.t_gpu_naive_kernel << "," << r.t_gpu_naive_d2h << ","
        << r.t_gpu_naive_total << "," << r.t_gpu_tiled_h2d << ","
        << r.t_gpu_tiled_kernel << "," << r.t_gpu_tiled_d2h << ","
        << r.t_gpu_tiled_total << "," << (r.validacao_ok ? "1" : "0") << "\n";
  }
  csv.close();
